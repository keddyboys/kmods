...
if (isset($_GET['photo_id']) && isnum($_GET['photo_id'])){
	...
	$result = dbquery(
		"SELECT album_title, album_description, album_thumb, album_parent, album_access FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$_GET['album_id']."'"
	);//subphotos
	if (!dbrows($result)) {
		redirect(FUSION_SELF);
	} else {
		...
		if (!checkgroup($data['album_access'])) {
			...
			if ($rows) {
				...
				//echo "<a href='".FUSION_SELF."'>".$locale['400']."</a> >\n";
            //subphotos begin
            if ($data['album_parent'] != 0 ) {
                $subdata = dbarray(dbquery("SELECT album_id, album_title FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$data['album_parent']."'"));
               echo "<a href='".FUSION_SELF."?album_id=".$subdata['album_id']."'>".$subdata['album_title']."</a> &gt;\n";
               echo "<a href='".FUSION_SELF."?album_id=".$_GET['album_id']."'>".$data['album_title']."</a>\n";    
            } else {
	            echo "<a href='".FUSION_SELF."?album_id=".$_GET['album_id']."'>".$data['album_title']."</a>\n";
            }
	...
	if ($rows) {
		$result = dbquery(
			...
			WHERE ".groupaccess('album_access')." AND album_parent='0' ORDER BY album_order
		...
		while ($data = dbarray($result)) {
			...
			echo "</a><br /><br />\n<span class='small'>\n";
			$sub_articles = dbcount("(album_id)", DB_PHOTO_ALBUMS, groupaccess('album_access')." AND album_parent='".$data['album_id']."'");//subphotos
			echo "</a><br /><br />\n<span class='small'>\n";//subphotos
			echo $locale['407'].": ".$sub_articles."<br />";//subphotos
