	...
	if (isset($_POST['move_thread'])) {
		...
		if (dbrows($result) != 0) {
			while ($data = dbarray($result)) {
				$result2 = dbquery("SELECT forum_id, forum_name FROM ".DB_FORUMS." WHERE forum_cat='".$data['forum_id']."' AND forum_parent='0' AND ".groupaccess('forum_access')." ORDER BY forum_order");//subforums
				if (dbrows($result2) != 0) {
