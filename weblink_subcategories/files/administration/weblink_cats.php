...
require_once INCLUDES."subcats_weblinks_include.php";//subweblinks
...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	if (isset($_POST['save_cat'])) {
		...
		$cat_parent = isnum($_POST['cat_parent']) ? $_POST['cat_parent'] : "0";//subweblinks
		...
		if ($cat_name) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
				$result = dbquery("UPDATE ".DB_WEBLINK_CATS." SET weblink_cat_name='$cat_name', weblink_cat_description='$cat_description', weblink_cat_sorting='$cat_sorting', weblink_cat_parent='$cat_parent', weblink_cat_access='$cat_access' WHERE weblink_cat_id='".$_GET['cat_id']."'");//subweblinks
				redirect(FUSION_SELF.$aidlink."&status=su");
			} else {
				$checkCat = dbcount("(weblink_cat_id)", DB_WEBLINK_CATS, "weblink_cat_name='".$cat_name."'");
				if ($checkCat == 0) {
					$result = dbquery("INSERT INTO ".DB_WEBLINK_CATS." (weblink_cat_name, weblink_cat_description, weblink_cat_sorting, weblink_cat_parent, weblink_cat_access) VALUES ('$cat_name', '$cat_description', '$cat_sorting', '$cat_parent', '$cat_access')"); //subweblinks
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
		$result = dbquery("SELECT weblink_cat_name, weblink_cat_description, weblink_cat_sorting, weblink_cat_parent, weblink_cat_access FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_id='".$_GET['cat_id']."'");//subweblinks
		if (dbrows($result)) {
			$data = dbarray($result);
			...
			$cat_parent = $data['weblink_cat_parent'];//subweblinks
	...
	echo "<td class='tbl'><select name='cat_parent' class='textbox' style='width:150px;'>\n".(isset($_GET['cat_id']) ? weblink_admin_editlist($_GET['cat_id']) : weblink_admin_editlist(null))."</select></td>\n";//subweblinks
	echo "</tr>\n<tr>\n";
	...
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_description, weblink_cat_access FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='0' ORDER BY weblink_cat_name");//subweblinks
	if (dbrows($result) != 0) {
		$i = 0;
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo "</tr>\n";
			echo weblink_admin_subcats($data['weblink_cat_id']); //subweblinks
			$i++;
		}
		echo "</table>\n";
	} else {
