...
if (!empty($result)) {
	...
	$editlist = ""; $sel = "";
	$result2 = dbquery("SELECT weblink_cat_id, weblink_cat_name FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='0' ORDER BY weblink_cat_name");//subweblinks
	if (dbrows($result2) != 0) {
	...
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_parent='0' ORDER BY weblink_cat_name");//subweblinks
	if (dbrows($result)) {
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo weblink_admin_subsubcats($data['weblink_cat_id']); // subweblinks
		}
		echo "</table>\n";
	}
	closetable();
