...
if (isset($_GET['article_id'])&& isnum($_GET['article_id'])){
	$result = dbquery(
		"SELECT ta.article_cat, tac.article_cat_name, tac.article_cat_parent, ta.article_id, ta.article_subject FROM ".DB_ARTICLES." ta
	...
	if (dbrows($result)) {
		...
		//subarticles		
		//$str .= $data['article_pat_parent'];
		if ($data['article_cat_parent'] != 0) {
		$data2 = dbarray(dbquery("SELECT article_cat_id, article_cat_name FROM ".DB_ARTICLE_CATS." WHERE article_cat_id='".$data['article_cat_parent']."'"));	
		$str .=	" &raquo; <a href='".FUSION_SELF."?cat_id=".$data2['article_cat_id']."'>".$data2['article_cat_name']."</a>";
		//&raquo; <a href='".FUSION_SELF."?cat_id=".$_GET['cat_id']."'>".$data['article_cat_name']."</a>";
	...
	$result = dbquery(
		"SELECT article_cat_name, article_cat_parent FROM ".DB_ARTICLE_CATS." 
		WHERE article_cat_id='".$_GET['cat_id']."'");//subarticles	
	if (dbrows($result)) {
		$data = dbarray($result);
		$str .= "<a href='".FUSION_SELF."'><strong>".$locale['404']."</strong></a>";
        //subarticles begin		
		if ($data['article_cat_parent'] != 0) {
		$data2 = dbarray(dbquery("SELECT article_cat_id, article_cat_name FROM ".DB_ARTICLE_CATS." WHERE article_cat_id='".$data['article_cat_parent']."'"));	
		$str .=	" &raquo; <a href='".FUSION_SELF."?cat_id=".$data2['article_cat_id']."'>".$data2['article_cat_name']."</a> &raquo; <a href='".FUSION_SELF."?cat_id=".$_GET['cat_id']."'>".$data['article_cat_name']."</a>";
		} else {
		$str .= " &raquo; <a href='".FUSION_SELF."?cat_id=".$_GET['cat_id']."'>".$data['article_cat_name']."</a>";
		}
		//subarticles end
...
if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
	...
	$result = dbquery(
		...
		WHERE article_cat_parent='0' AND ".groupaccess('ac.article_cat_access')."
	...
	if ($rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo article_subcats($data['article_cat_id'], $type="2");//subarticles
			echo "</td>\n";
			$counter++;
		}
		echo "</tr>\n</table>\n";
	...
	$res = 0;
	$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_sorting, article_cat_access FROM ".DB_ARTICLE_CATS." WHERE article_cat_id='".$_GET['cat_id']."'");
	if (dbrows($result) != 0) {
		...
		if (checkgroup($cdata['article_cat_access'])) {
	        ...
	        echo article_subcats($cdata['article_cat_id'], $type="1");//subarticles	
