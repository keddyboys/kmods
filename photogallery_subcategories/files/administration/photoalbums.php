...
if (function_exists('gd_info')) {
	...
	if (isset($_GET['action']) && $_GET['action'] == "refresh") {
		$i = 1; $k = 1;
		$result = dbquery("SELECT album_id FROM ".DB_PHOTO_ALBUMS." WHERE album_parent='0' ORDER BY album_order");//subphotos
	...
	if (isset($_POST['cancel'])) {
		...
		$album_parent = isnum($_POST['album_parent']) ? $_POST['album_parent'] : "0";//subphotos
		...
		if (!$error) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
				...
				$result = dbquery("UPDATE ".DB_PHOTO_ALBUMS." SET album_title='$album_title', album_description='$album_description',".(isset($album_thumb)?" album_thumb='$album_thumb',":"")." album_user='".$userdata['user_id']."', album_parent='$album_parent', album_access='$album_access', album_order='$album_order' WHERE album_id='".$_GET['album_id']."'");//subphotos
				...
				$result = dbquery("INSERT INTO ".DB_PHOTO_ALBUMS." (album_title, album_description, album_thumb, album_user, album_parent, album_access, album_order, album_datestamp) VALUES ('$album_title', '$album_description', '".(isset($album_thumb) ? $album_thumb : "")."', '".$userdata['user_id']."', '$album_parent', '$album_access', '$album_order', '".time()."')");//subphotos
				$rowstart = $album_order > $settings['thumbs_per_page'] ? ((ceil($album_order / $settings['thumbs_per_page'])-1)*$settings['thumbs_per_page']) : "0";
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['album_id']) && isnum($_GET['album_id']))) {
			$result = dbquery("SELECT album_title, album_description, album_thumb, album_parent, album_access, album_order FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$_GET['album_id']."'");//subphotos
			if (dbrows($result)) {
				$data = dbarray($result);
				...
				$album_parent = $data['album_parent']; //subphotos
        ...
        echo "<td class='tbl'><select name='album_parent' class='textbox' style='width:150px;'>\n".(isset($_GET['album_id']) ? photo_admin_editopts($_GET['album_id']) : photo_admin_editopts(null))."</select></td>\n";// subcategories
		echo "</tr>\n<tr>\n";
	...
	$rows = dbcount("(album_id)", DB_PHOTO_ALBUMS, "album_parent='0'");//subphotos
	if ($rows) {
		...
		$result = dbquery(
			...
			LEFT JOIN ".DB_USERS." tu ON ta.album_user=tu.user_id WHERE album_parent='0'
		...
		while ($data = dbarray($result)) {
			...
			$sub = dbcount("(album_id)", DB_PHOTO_ALBUMS, "album_parent='".$data['album_id']."'"); //subphotos
			echo "<br /><br />\n".$locale['447'].$sub."<br />"; //Subphotos
