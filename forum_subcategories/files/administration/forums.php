...
require_once INCLUDES."subcats_forums_include.php";//subforums
...
if (isset($_POST['save_cat'])) {
	...
	$forum_parent = isnum($_POST['forum_parent']) ? $_POST['forum_parent'] : 0;//subforums
	if ($forum_name != "") {
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['forum_id']) && isnum($_GET['forum_id'])) && (isset($_GET['t']) && $_GET['t'] == "forum")) {
			...
			if(!$forum_order) $forum_order=dbresult(dbquery("SELECT MAX(forum_order) FROM ".DB_FORUMS." WHERE forum_parent='$forum_parent' AND forum_cat='$forum_cat'"),0)+1;//subforums
			$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order+1 WHERE forum_cat='$forum_cat' AND forum_parent='$forum_parent' AND forum_order>='$forum_order'");//subforums
			$result = dbquery("INSERT INTO ".DB_FORUMS." (forum_cat, forum_parent, forum_name, forum_order, forum_description, forum_moderators, forum_access, forum_post, forum_reply, forum_attach, forum_attach_download, forum_poll, forum_vote, forum_lastpost, forum_lastuser, forum_merge) VALUES ('".$forum_cat."', '".$forum_parent."', '".$forum_name."', '".$forum_order."', '".$forum_description."', '103', '101', '101', '101', '0', '0', '0', '0', '0', '0', '0')");//subforums
	...
	if (isset($_GET['t']) && $_GET['t'] == "cat") {
		$parent = (isset($_GET['parent']) && isNum($_GET['parent']) ? " AND forum_parent='".$_GET['parent']."'" : "");//subforums
		$data = dbarray(dbquery("SELECT forum_id FROM ".DB_FORUMS." WHERE forum_cat='0' AND forum_order='".$_GET['order']."'$parent"));//subforums
		...
		$parent = (isset($_GET['parent']) && isNum($_GET['parent']) ? " AND forum_parent='".$_GET['parent']."'" : "");//subforums
		$data = dbarray(dbquery("SELECT forum_id FROM ".DB_FORUMS." WHERE forum_cat='".$_GET['cat']."' AND forum_order='".$_GET['order']."'$parent"));//subforums
		$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order+1 WHERE forum_id='".$data['forum_id']."'");
		$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order-1 WHERE forum_id='".$_GET['forum_id']."'");
	}
	redirect(FUSION_SELF.$aidlink);
	...
	if ((!dbcount("(thread_id)", DB_THREADS, "forum_id='".$_GET['forum_id']."'"))&& (!dbcount("(forum_id)", DB_FORUMS."", "forum_parent='".$_GET['forum_id']."'"))) { //subforums {
		$parent = (isset($_GET['parent']) && isNum($_GET['parent']) ? " AND forum_parent='".$_GET['parent']."'" : "");//subforums
		$data = dbarray(dbquery("SELECT forum_cat, forum_order FROM ".DB_FORUMS." WHERE forum_id='".$_GET['forum_id']."'"));
		$result = dbquery("UPDATE ".DB_FORUMS." SET forum_order=forum_order-1 WHERE forum_cat='".$data['forum_cat']."' AND forum_order>'".$data['forum_order']."'$parent");//subforums
		$result = dbquery("DELETE FROM ".DB_FORUMS." WHERE forum_id='".$_GET['forum_id']."'");
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['forum_id']) && isnum($_GET['forum_id']))) {
		if (isset($_GET['t']) && $_GET['t'] == "cat") {
			...
			$result = dbquery(
				"SELECT
					forum_name, forum_parent, forum_description, forum_cat, forum_moderators,
			...
			if (dbrows($result)) {
				...
				$forum_parent = $data['forum_parent'];//subforums
