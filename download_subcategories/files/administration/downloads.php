...
if (!empty($result)) {
	...
	if (dbrows($result2) != 0) {
		while ($data2 = dbarray($result2)) {
			...
			$editlist .= download_subdownloads($data2['download_cat_id']);//subdownloads
		}
	}
	...
	$result = dbquery("SELECT download_cat_id, download_cat_name FROM ".DB_DOWNLOAD_CATS." WHERE download_cat_parent='0' ORDER BY download_cat_name");
	if (dbrows($result)) {
		echo "<tr>\n";
