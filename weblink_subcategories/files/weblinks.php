...
if (!isset($_GET['cat_id']) || !isnum($_GET['cat_id'])) {
	...
	$result = dbquery("SELECT weblink_cat_id, weblink_cat_name, weblink_cat_description FROM ".DB_WEBLINK_CATS." WHERE ".groupaccess('weblink_cat_access')." AND weblink_cat_parent='0' ORDER BY weblink_cat_name");//subweblinks
	$rows = dbrows($result);
	if ($rows != 0) {
		$counter = 0; $columns = 2; 
		...
		while ($data = dbarray($result)) {
			...
			weblink_subcats($data['weblink_cat_id'], "1"); // subweblinks	
			echo "</td>\n";
			$counter++;
		}
		echo "</tr>\n</table>\n";
	...
	if (dbrows($result) != 0) {
		...
		if (checkgroup($cdata['weblink_cat_access'])) {
			...
			echo weblink_subcats($_GET['cat_id'], '2'); // subweblinks
			if ($rows != 0) {
