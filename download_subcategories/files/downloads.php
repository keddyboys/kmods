...
if (!isset($_GET['download_id']) || !isnum($_GET['download_id'])) {
	...
	$cat_list_result = dbquery(
		...
		ORDER BY download_cat_name");//subdownloads
	...
	if (dbrows($cat_list_result)) {
		while ($cat_list_data = dbarray($cat_list_result)) {
			...
			$cats_list .= download_subdownloads($cat_list_data['download_cat_id']);//subdownloads
		}
		...
		if (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) {
			...
			echo "</td>\n";
			echo "</tr>\n";
			echo download_subcats($_GET['cat_id'], '2'); //subdownloads
        ...
        $parent = (isset($_GET['cat_id']) && isnum($_GET['cat_id'])) ? "" : " AND download_cat_parent='0'";//subdownloads
	$cat_result = dbquery(
			...
			WHERE ".groupaccess('download_cat_access').$parent.$filter."
			ORDER BY download_cat_name");//subdownloads
