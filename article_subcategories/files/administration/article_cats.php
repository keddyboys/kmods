...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	if (isset($_POST['save_cat'])) {
		...
		if ($cat_name) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
				$result = dbquery("UPDATE ".DB_ARTICLE_CATS." SET article_cat_name='$cat_name', article_cat_description='$cat_description', article_cat_sorting='$cat_sorting', article_cat_parent='$cat_parent', article_cat_access='$cat_access' WHERE article_cat_id='".$_GET['cat_id']."'");//subarticles
				redirect(FUSION_SELF.$aidlink."&status=su");
			} else {
				$checkCat = dbcount("(article_cat_id)", DB_ARTICLE_CATS, "article_cat_name='".$cat_name."'");
				if ($checkCat == 0) {
					$result = dbquery("INSERT INTO ".DB_ARTICLE_CATS." (article_cat_name, article_cat_description, article_cat_sorting, article_cat_parent, article_cat_access) VALUES ('$cat_name', '$cat_description', '$cat_sorting', '$cat_parent', '$cat_access')");//subarticles
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
		...
		if (dbrows($result)) {
			...
			$cat_parent = $data['article_cat_parent'];//subarticles
	...
	$result = dbquery("SELECT article_cat_id, article_cat_name, article_cat_description, article_cat_access FROM ".DB_ARTICLE_CATS." WHERE article_cat_parent='0' ORDER BY article_cat_name");//subarticles
	if (dbrows($result) != 0) {
		$i = 0;
		echo "<tr>\n";
		...
		while ($data = dbarray($result)) {
			...
			echo "</tr>\n";
			echo articles_admin_subcats($data['article_cat_id']);//subarticles
			$i++;
		}
		echo "</table>\n";
	} else {
